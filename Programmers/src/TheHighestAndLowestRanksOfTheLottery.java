...
class TheHighestAndLowestRanksOfTheLotteryTest {
    ...
    @Test
    public void test() {
        TheHighestAndLowestRanksOfTheLottery problem = new TheHighestAndLowestRanksOfTheLottery();
        ...
        IntStream.range(0, expectedResult.length)
                .forEach(i -> assertThat(expectedResult[i]).isEqualTo(problem.solution(lottos[i], win_nums[i])));
    }
}
